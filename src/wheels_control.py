...
# SP = Set Point (giá trị mong muốn)
# PV = Process Value (giá trị đo được)
...
class wheels_control(Node):
    ...
    def SP_velo_callback(self, msg):
        ...
        self.get_logger().info(f"V = {SP_linear_vel}, W = {SP_angular_vel}")

        if round(SP_linear_vel,10) == 0.0 and round(SP_angular_vel,10) == 0.0:
            self.stop = True
            self.SP_left = 0.0
            self.SP_right = 0.0
            ...
            SP_left = SP_linear_vel - self.wheels_distance * SP_angular_vel / 2  # m/s
            SP_right = SP_linear_vel + self.wheels_distance * SP_angular_vel / 2
            self.SP_left = np.clip(SP_left,-0.5,0.5)
            self.SP_right = np.clip(SP_right,-0.5,0.5)
    ...
    def PID_control(self):
        if self.received_vel and self.stop == False:
            #tinh sai so
            ...
            self.get_logger().info(f"l = {self.left_pwm}, r = {self.right_pwm}")
