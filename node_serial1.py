...
class ConnectNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('pulses_per_revolution', 2000.0) # che do doc rising 
    ...
    def odom(self):
        ...
        Dl = self.en1 * self.scaling_factor
        Dr = self.en2 * self.scaling_factor
        Davg = (Dl + Dr) / 2
        Delta_theta = (Dr - Dl) / self.wheels_distance
        ...
        self.get_logger().info(f"x={odom_msg.pose.pose.position.x}, y={odom_msg.pose.pose.position.y}, theta={odom_msg.pose.pose.orientation}")
    ...
    def velocities_callback(self, msg):
        self.write_serial(1, msg.data[0], msg.data[1])
